...
.controller('HistoricalController', function ($scope, $q, HService) {
  ...
  ****************/
  ...
  function initController(){
    ...
    vm.selectedStartDate = '2017-10-18T10:00:00'

    vm.selectedEndDate = '2017-11-18T10:00:00'

    callSensorReadings ("6901");
  }//end initController
    ...
    .then(function(result){
      ...
      vm.calendarheatmapdata=angular.copy(calendar_data);
        ...
        result.results.forEach(function(value,index){
          ...
          var gw_timestamp = moment(value.gw_timestamp).format("YYYY-MM-DD");
          if(date_storage.indexOf(gw_timestamp)== -1){
            date_storage.push(gw_timestamp);
            calendar_data.push({
              "date": ""+ gw_timestamp,
              "total": 0,
              "details": []
            })
            var index = data_storage.indexOf(gw_timestamp);
            calendar_data[index].details.push({
              "name": id,
              "date": moment(value.gw_timestamp).format("YYYY-MM-DD HH:mm:ss"),
              "value":
            })
          }//end if
        })//end foreach
